...
function renderAuth() {
  ...
  ui.loginForm.classList.toggle("hidden", isAdmin);
  ui.residentModeBtn.classList.toggle("hidden", auth.role === "resident");
  ui.logoutBtn.classList.toggle("hidden", isGuest);

  if (isAdmin) {
    ui.authState.textContent = "מצב משתמש: מנהל (admin)";
  } else if (auth.role === "resident") {
    ui.authState.textContent = "מצב משתמש: דייר";
  } else {
    ui.authState.textContent = "מצב משתמש: לא מחובר";
  }
}
